...
//
//	/* 原始内容  */
//	//mCommandList->SetPipelineState(mPSOs["denyRG"].Get());
//	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opaque]);
//
//	// 绘制顶点法线
//	//mCommandList->SetPipelineState(mPSOs["normal"].Get());
//	//DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Normal]);
//
...
//
//	mShaders["normalVS"] = d3dUtil::CompileShader(L"Shaders\\subdivision.hlsl", nullptr, "VS", "vs_5_0");
//	mShaders["normalPS"] = d3dUtil::CompileShader(L"Shaders\\subdivision.hlsl", nullptr, "PS", "ps_5_0");
//	mShaders["normalGS"] = d3dUtil::CompileShader(L"Shaders\\subdivision.hlsl", nullptr, "GS", "gs_5_0");
//	
//    mInputLayout =
//    {
...
//}
//
//void BlendApp::BuildStellationGeometry() {
//	GeometryGenerator geoGen;
//	GeometryGenerator::MeshData stellation = geoGen.CreateGeosphere20Face(5.0f);
//
//	std::vector<Vertex> vertices(12);//初始化顶点列表
//	//顶点列表赋值
//	for (UINT i = 0; i < 12; i++)
//	{
//		vertices[i].Pos = stellation.Vertices[i].Position;
//		vertices[i].Normal = stellation.Vertices[i].Normal;
//		vertices[i].TexC = stellation.Vertices[i].TexC;
//	}
//
...
//	for (UINT i = 0; i < 60; i++)
//	{
//		indices[i] = stellation.Indices32[i];
//	}
//
...
//	CopyMemory(geo->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);
//
//	geo->DrawArgs["stellation"] = geoSphereSubmesh;
//
//	mGeometries["geoStellation"] = std::move(geo);
//}
//
...
//	};
//	auto raster_desc = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
//	raster_desc.FillMode = D3D12_FILL_MODE_WIREFRAME;
//	//raster_desc.CullMode = D3D12_CULL_MODE_NONE;
//	opaquePsoDesc.RasterizerState = raster_desc;
//	opaquePsoDesc.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);  // 设置混合状态
...
//
//	//
//	// PSO ---> 禁止向 R 通道 和 G 通道 写入颜色
//	//
//	D3D12_GRAPHICS_PIPELINE_STATE_DESC denyRGPsoDesc = opaquePsoDesc;
//
//	D3D12_RENDER_TARGET_BLEND_DESC denyRGBlendDesc;
//	denyRGBlendDesc.BlendEnable = true;
//	denyRGBlendDesc.LogicOpEnable = false;
//	denyRGBlendDesc.SrcBlend = D3D12_BLEND_SRC_ALPHA;
//	denyRGBlendDesc.DestBlend = D3D12_BLEND_INV_SRC_ALPHA;
//	denyRGBlendDesc.BlendOp = D3D12_BLEND_OP_ADD;
//	denyRGBlendDesc.SrcBlendAlpha = D3D12_BLEND_ONE;
//	denyRGBlendDesc.DestBlendAlpha = D3D12_BLEND_ZERO;
//	denyRGBlendDesc.BlendOpAlpha = D3D12_BLEND_OP_ADD;
//	denyRGBlendDesc.LogicOp = D3D12_LOGIC_OP_NOOP;
//	denyRGBlendDesc.RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_BLUE | D3D12_COLOR_WRITE_ENABLE_ALPHA;
//
//	denyRGPsoDesc.BlendState.RenderTarget[0] = denyRGBlendDesc;
//	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&denyRGPsoDesc, IID_PPV_ARGS(&mPSOs["denyRG"])));
//
//	D3D12_GRAPHICS_PIPELINE_STATE_DESC depthComplexityPsoDesc = opaquePsoDesc;
//	CD3DX12_BLEND_DESC depthCompBlend(D3D12_DEFAULT);
//	depthCompBlend.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_RED;
//
...
//	depthComplexityPsoDesc.BlendState = depthCompBlend;
//
//	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&depthComplexityPsoDesc, IID_PPV_ARGS(&mPSOs["depthComplexity0"])));
//
...
//	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&depthComplexityPsoDesc2, IID_PPV_ARGS(&mPSOs["depthComplexity2"])));
//
...
//	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&depthComplexityPsoDesc3, IID_PPV_ARGS(&mPSOs["depthComplexity3"])));
//
//
//	//
//	// PSO FOR NORMAL (法线)
...
//
//	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&normalPsoDesc, IID_PPV_ARGS(&mPSOs["normal"])));
//}
//
...
//	XMStoreFloat4x4(&wavesRitem->TexTransform, XMMatrixScaling(5.0f, 2.0f, 1.0f));
...
//	XMStoreFloat4x4(&sphereRitem->World, XMMatrixTranslation(3.0f, 4.0f, -9.0f));
//	sphereRitem->ObjCBIndex = 2;
//	sphereRitem->Mat = mMaterials["wirefence"].get();
//	sphereRitem->Geo = mGeometries["geoStellation"].get();
//	sphereRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
//	sphereRitem->IndexCount = sphereRitem->Geo->DrawArgs["stellation"].IndexCount;
//	sphereRitem->StartIndexLocation = sphereRitem->Geo->DrawArgs["stellation"].StartIndexLocation;
//	sphereRitem->BaseVertexLocation = sphereRitem->Geo->DrawArgs["stellation"].BaseVertexLocation;
//
//	mRitemLayer[(int)RenderLayer::Opaque].push_back(sphereRitem.get());
//
//	auto normalRitem = std::make_unique<RenderItem>();
//	*normalRitem = *sphereRitem;
//	XMStoreFloat4x4(&normalRitem->World, XMMatrixTranslation(3.0f, 4.0f, -9.0f));
//	normalRitem->ObjCBIndex = 3;
//	normalRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_POINTLIST;
//	//normalRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
//
//	mRitemLayer[(int)RenderLayer::Normal].push_back(normalRitem.get());
//
//
...
//	mAllRitems.push_back(std::move(normalRitem));
//}
//
...
//
//		cmdList->SetGraphicsRootDescriptorTable(0, tex);              // 绑定纹理
//        cmdList->SetGraphicsRootConstantBufferView(1, objCBAddress);  // 绑定物体常量
//        cmdList->SetGraphicsRootConstantBufferView(3, matCBAddress);  // 绑定材质常量
//

...
private:
	...
	XMFLOAT3 mfloorTranslation = { 0.0f, 0.0f, 0.0f };
...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	// update the reflection floor matrix
	XMMATRIX floorRotate = XMMatrixRotationY(0.5f * MathHelper::Pi);
	XMMATRIX floorOffset = XMMatrixTranslation(mfloorTranslation.x, mfloorTranslation.y, mfloorTranslation.z);
	XMMATRIX floorWorld = floorOffset;

	XMStoreFloat4x4(&mReflectedFloorRitem->World, floorWorld * R);//设置镜像地板的世界矩阵
	...
	// 先用skull的world矩阵变换坐标，再乘以阴影矩阵S投影到地面，再进行y轴偏移以防止 z-fighting
...
void StencilApp::BuildPSOs()
{
	...
	opaquePsoDesc.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT); // 绑定深度/模板状态
	...
	mirrorDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;  // 总是返回 true

	// 我们不渲染背面朝向的三角形，因此下面这些设置都无关紧要
	mirrorDSS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	mirrorDSS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	mirrorDSS.BackFace.StencilPassOp = D3D12_STENCIL_OP_REPLACE;
	mirrorDSS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;  // 总是返回 true
	...
	shadowDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;  // 等于0通过模板测试
...
void StencilApp::BuildRenderItems()
{
	...
	*reflectedFloorRitem = *floorRitem;
	reflectedFloorRitem->World = MathHelper::Identity4x4();
